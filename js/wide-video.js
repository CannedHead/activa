...
var dataVideos=[
	{filename:"golf",boxId:"slide1",color:"#353ff8",background:"#FFFFFF"},
	{filename:"golf1",boxId:"slide2",color:"#D0AA00",background:"#FFFFFF"}		
	];
...
function imageSlide(i){
	...
	if(count==1){
		if(!($("#image-"+load_i.filename).length>0)){
			$("#images").prepend('<div class="image-box" id="image-'+load_i.boxId+'" ><img class="image-slide" id="image-'+load_i.filename+'" src="img/widevideo/photos/'+load_i.filename+'.png" /></div>');
			sizeImage();
		}
	}
...
function loadMediaFirst(){
	if(!mobileCheck()){
			...
			$("#"+dataVideos[0].boxId).css("background-image","url(img/widevideo/photos/"+dataVideos[0].filename+".png)");
...
function loadMediaSecond(){
	if(!mobileCheck()){
		...
		$("#"+dataVideos[1].boxId).css("background-image","url(img/widevideo/photos/"+dataVideos[1].filename+".png)");
		...
		//$("#images").prepend('<div class="image-box" id="image-'+dataVideos[1].boxId+'" ><img class="image-slide" id="image-'+dataVideos[1].filename+'" src="img/widevideo/photos/'+dataVideos[1].filename+'.png" /></div>');sizeImage();
		$("#"+dataVideos[1].boxId).css("background-image","url(img/widevideo/photos/"+dataVideos[1].filename+".png)");
		hideVideo(dataVideos[1].filename,0);
		sizeVideo();
		sizeVideoBg();
...
function loopVideo(){
	...
	for(var i=0;i<dataVideos.length;i++){
		var delay=(i+1)*20000;setTimeout("videoSlide("+i+");",delay);
	}
}

function panningImage(id,ratio){
	$(id).animate({right:"5%"},4400);
}
...
function videoHtml(video){
	...
	if(!($("#"+dataVideos[0].filename).hasClass("hidden"))||force){
		...
		$("#"+video.filename).append('<source src="img/widevideo/videos/'+video.filename+'.mp4" type="video/mp4">');
		$("#"+video.filename).append('<source src="img/widevideo/videos/'+video.filename+'.ogv" type="video/ogg">');
		$("#"+video.filename).append('<source src="img/widevideo/videos/'+video.filename+'.webm" type="video/webm">');
	}
	else{
	...
	$("#"+video.filename).on("canplay",videoOnCanPlay);
	sizeVideo();
	sizeVideoBg();
}
...
function videoSlide(i){
	...
	if(count==1){
		...
		$("#"+load_v.boxId).css("background-image","url(img/widevideo/photos/"+load_v.filename+".png)");
	...
	playVideo(next_v);
	$("#"+next_v.boxId).css("opacity",1).css("zIndex",-2).animate({zIndex:-1},200).toggleClass("expand");
	$("#"+curr_v.boxId).css("opacity",0).css("zIndex",-1).animate({zIndex:-2},200).toggleClass("expand");
}
/*